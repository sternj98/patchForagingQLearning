# bring together environment and agent
...
class DeepRLInterface():
    ...
    def step(self,probe_trial = {},probe_action = -1):
        ...
        rewIdx = state["rewindex"]
        rewInt = self.agent.integrate(state) # internalize old env state
        patch = state["patch"]

        if probe_action < 0:
            action = self.agent.select_action(rewIdx,RewInt,patch) # agent selects action
        else:
            action = probe_action
        ...
        newRewIdx = self.env.state["rewindex"]
        newRewInt = self.agent.integrate(new_state) # internalize new env state
        newPatch = new_state["patch"]

        self.agent.replay_buffer.push(state,action,new_state,rew) # add our memory to exp buffer
